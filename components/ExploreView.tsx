...
import { Search, MapPin, ArrowUpRight, Filter, Star, X, ArrowLeft, Heart, Share2, Plus, Send, ChevronRight, Layers, Navigation, Minus } from 'lucide-react';
...
// Enriched Mock Data with "Map Positions" (simulating lat/lng with % for the prototype map)
const MOCK_DESTINATIONS: (ExploreItem & { mapPos: { top: string; left: string } })[] = [
  {
    id: '1',
    title: 'Bistro Järva',
    subtitle: 'Just pleasure',
    category: 'Restaurant',
    images: [
  ...
  },
  {
    ...
    category: 'Experience',
    images: [
        'https://images.unsplash.com/photo-1512428559087-560fa0cec34f?auto=format&fit=crop&q=80&w=400',
    ],
    tags: ['Museum', 'Family'],
    rating: 4.7,
    reviewCount: 4200,
...
export const ExploreView: React.FC<ExploreViewProps> = ({ onPlanTrip, onAddItem, onToggleSave, savedItemIds }) => {
  ...
  return (
    <div className="h-full relative bg-gray-50 flex overflow-hidden">
      ...
      <div className="w-full lg:w-[45%] xl:w-[40%] flex flex-col h-full bg-white border-r border-gray-200 z-10 shadow-xl lg:shadow-none">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-gray-100 bg-white z-20">
          <div className="flex items-center gap-2 mb-4">
             <h1 className="text-xl font-bold text-slate-900">Current location</h1>
          ...
          <div className="flex gap-3 mb-4">
            <div className="relative flex-1">
                ...
                type="text" 
                placeholder="Search"
                className="w-full pl-10 pr-4 py-2.5 bg-gray-100 border-none rounded-full focus:ring-2 focus:ring-slate-900/10 focus:bg-white transition-all text-sm font-medium text-slate-900"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                />
            </div>
            <button className="flex items-center gap-2 px-4 py-2.5 rounded-full border border-gray-200 text-sm font-semibold hover:bg-gray-50 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white">
            <h2 className="text-lg font-bold text-slate-900 mb-4">{activeCategory}</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-4 pb-20">
                {filteredItems.map(item => (
                <div 
                    key={item.id} 
                    className={`group bg-white rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 border
                        ${hoveredItemId === item.id ? 'shadow-lg border-gray-300 ring-1 ring-gray-200' : 'shadow-sm border-gray-100'}
                    `}
                ...
                >
                    <div className="h-40 overflow-hidden relative">
                        ...
                        />
                        <button className="absolute top-2 right-2 p-1.5 bg-black/30 hover:bg-black/50 backdrop-blur-md rounded-full text-white transition-colors">
                            <Heart size={14} fill={savedItemIds.has(item.id) ? "currentColor" : "none"} />
                        </button>
                         <button className="absolute top-2 right-10 p-1.5 bg-black/30 hover:bg-black/50 backdrop-blur-md rounded-full text-white transition-colors">
                            <Plus size={14} />
                        </button>
                    </div>
                    ...
                    <div className="p-4">
                        <div className="flex justify-between items-start mb-1">
                            ...
                            <div className="flex items-center gap-1 text-xs font-bold text-slate-900">
                                <Star size={10} className="text-slate-900 fill-current" />
                                {item.rating} <span className="text-gray-400 font-normal">({item.reviewCount})</span>
                            </div>
                        </div>
                        <p className="text-xs text-gray-500 line-clamp-1 mb-2">{item.subtitle} • {item.priceLevel}</p>
      ...
      <div className="hidden lg:block flex-1 bg-gray-200 relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gray-100">
             {/* We use a static map image of Stockholm area to match prototype fidelity without API Key */}
             <div 
                ...
                style={{
                    ...
                    filter: 'grayscale(10%) contrast(95%) brightness(105%) opacity(0.8)',
             ...
             {MOCK_DESTINATIONS.map((item) => {
                 ...
                 return (
                    ...
                    >
                        <div className={`
                            flex items-center gap-1.5 px-3 py-1.5 rounded-full shadow-md font-bold text-xs border transition-colors
                            ${isActive 
                        ...
                        `}>
                            {item.category === 'Hotel' ? (
                                ...
                                <>
                                 ...
                                 <span className={isActive ? 'text-white' : ''}>{item.priceLevel}</span>
                                </>
                            )}
                        </div>
                        {/* Triangle pointer */}
                        <div className={`
                            ...
                            ${isActive ? 'bg-slate-900 border-slate-900' : 'bg-white border-gray-200'}
                        `}></div>
                    </div>
                 );
             })}
        ...
        <div className="absolute top-4 left-4 z-10">
            <button className="px-4 py-2 bg-white rounded-full shadow-md text-sm font-semibold hover:bg-gray-50 text-slate-900 border border-gray-200">
                Search this area
            </button>
        </div>

        <div className="absolute bottom-8 right-4 z-10 flex flex-col gap-2">
            <button className="w-10 h-10 bg-white rounded-lg shadow-md flex items-center justify-center text-gray-600 hover:text-slate-900 border border-gray-200">
                <Navigation size={18} />
            </button>
            <div className="bg-white rounded-lg shadow-md border border-gray-200 flex flex-col">
                <button className="w-10 h-10 flex items-center justify-center text-gray-600 hover:text-slate-900 border-b border-gray-100">
                    <Plus size={18} />
                </button>
                <button className="w-10 h-10 flex items-center justify-center text-gray-600 hover:text-slate-900">
                    <Minus size={18} />
                </button>
            </div>
        </div>
        ...
        <div className="absolute top-4 right-4 z-10">
             <button className="w-10 h-10 bg-white rounded-full shadow-md flex items-center justify-center text-gray-600 hover:text-slate-900 border border-gray-200">
                <Layers size={18} />
            </button>
        </div>
      </div>
      ...
      <div 
        className={`
          fixed inset-y-0 right-0 w-full md:w-[500px] bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col
          ...
          <>
            <div className="flex items-center justify-between p-4 border-b border-gray-100 bg-white">
              ...
              <div className="flex gap-2">
                ...
                >
                  <Heart size={16} fill={savedItemIds.has(selectedItem.id) ? "currentColor" : "none"} /> 
                  Save
                </button>
                <button 
                    onClick={() => onAddItem(selectedItem)}
                    className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-900 text-white text-sm font-bold hover:bg-slate-800 shadow-sm"
                >
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="p-6 pb-32">
                ...
                <h1 className="text-2xl font-bold text-slate-900 mb-1">{selectedItem.title}</h1>
                <p className="text-slate-500 text-lg mb-4">{selectedItem.subtitle}</p>
                
                <div className="flex items-center gap-3 text-sm text-gray-600 mb-6">
                    <span className="flex items-center gap-1 font-bold text-slate-900"><Star className="text-amber-500 fill-amber-500" size={14} /> {selectedItem.rating}</span>
                    <span className="text-gray-400">({selectedItem.reviewCount})</span>
                    <span>•</span>
                    <span>{selectedItem.priceLevel}</span>
                    <span>•</span>
                    <span>{selectedItem.category}</span>
                </div>

                <div className="aspect-video w-full rounded-2xl overflow-hidden mb-8 relative bg-gray-100 group">
                    ...
                    <button className="absolute bottom-4 right-4 bg-white/90 backdrop-blur px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm">
                        View all photos
                    </button>
                </div>
                ...
                <div className="animate-in fade-in duration-300">
                        ...
                        <div>
                            ...
                            <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 mb-4">
                                ...
                                <div className="flex items-center text-sm text-gray-500">
                                    <MapPin size={16} className="mr-2" /> {selectedItem.address}
                                </div>
                            </div>
                        </div>
                    )}
                    {activeTab === 'reviews' && (
                        <div className="text-center py-10 text-gray-400">
                            <p>Reviews integrated from Google Maps.</p>
                        </div>
                    )}
                    {activeTab === 'location' && (
                         <div className="text-center py-10 text-gray-400">
            ...
            <div className="absolute bottom-6 left-6 right-6 z-20">
               <form 
                 ...
                 className="bg-white p-2 rounded-full shadow-2xl border border-gray-100 flex items-center gap-2 ring-1 ring-black/5"
               >
                  <input 
                    type="text" 
                    placeholder={`Ask about ${selectedItem.title}...`}
                  ...
                  <button 
                    type="submit"
                    className="w-8 h-8 flex items-center justify-center bg-slate-900 rounded-full text-white hover:bg-slate-800 transition-all disabled:opacity-50"
